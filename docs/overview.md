...
Alt+V（keyboard 全局热键，常驻后台）
  ...
  → 停顿检测（默认 0.8s）或 20s 识别流重置 → 标点模型只向当前段插入标点并锁段
...
- 保留最长公共前缀，只退格并重写不稳定尾部；endpoint 触发后锁段不再变动，进入校对队列
- 20 秒 rule3 endpoint 只表示识别器必须重置，不意味着用户说完一句。按本段接受的音频时长
  保守判定这种切段，允许内部标点但不在段尾强加句号；用户可以继续说话，无需配合停顿。
...
- 模型可参考最近 32 字已锁定原文，但这部分只读，投影只保留当前段的新增标点。上下文导致
  模型改写原文时回退单段模式；强制切段时移除模型猜测的句末符号。
- Qwen 仍在停止后负责错字、语气词和通顺度，轻量标点层不承担文字改写

### 为什么校对放在停止后而不是边说边校
- 边说边校对会反复替换光标处文本，与用户编辑冲突、画面跳动（用户决策：听写期间屏幕文字只增不改）
- 自然停顿先恢复标点并锁段；Alt+V 停止后把锁定段按约 60 字分块，逐块校对并替换。
  每块额外读取前后各至多 24 字原文作为只读语境，输出只能对应本块；若后面仍有连续内容，
  校对结果不得在该块末尾强加句号。
