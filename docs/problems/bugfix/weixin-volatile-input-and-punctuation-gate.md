...
已确认：微信的活动富文本组合区对句末追加也会偶发保留上一轮符号，形成连续问号。只要在
partial 实时上屏后继续修改同一组合区，就无法保证屏幕文本与内部记账一致。
...
`python scripts/check_session.py` 通过 51 项测试，其中包含微信动态 UIA、锁句最终文本丢字、
句首/重复标点，以及微信端点缓冲模式在 partial 阶段零写入、endpoint 仅一次慢速整句写入、
校对阶段零覆盖。尚未在真实微信客户端完成本轮端点缓冲模式的手工冒烟。
