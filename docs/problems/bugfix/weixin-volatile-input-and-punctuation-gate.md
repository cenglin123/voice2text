...
已确认：微信富文本编辑器对向前移动光标后的 Unicode 标点插入也不可靠；即使不发送退格和
不重打汉字，插入点之后的首字仍可能被覆盖。因此微信不能使用任何回溯式句中编辑。
...
7. 微信启用保守兼容模式：每次锁句只在当前光标处追加一个句末符号，不移动光标；停止后的
   二次校对若需要修改已有内容则保留原文并报告未应用。其他已验证编辑器仍保留句中标点。

## 验证结果

`python scripts/check_session.py` 通过 50 项测试，其中包含微信动态 UIA、锁句最终文本丢字、
句首/重复标点、“锄禾日当午”样例零退格，以及微信兼容模式零导航、零退格、零整段替换。
尚未在真实微信客户端完成本轮兼容模式的手工冒烟。
