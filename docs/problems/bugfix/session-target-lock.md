...
2026-09-19 取消诊断补充：校对前后现在输出取消原因、会话编号、阶段及已发生的推理耗时。
此前仅输出 cancelled，无法区分手动输入、焦点失败和超时；没有推理耗时记录也不代表没有推理。
输入保护使用本钩子收到的物理按下/松开事件跟踪热键修饰键，避免异步键状态不一致时
将停止 Alt+V 误判为手动编辑。回归模拟异步状态为未按下，验证 Alt+V 豁免、松开 Alt 后
单独 V 仍关闸。用户本次实际取消来源仍需新版日志确认，不能仅凭此回归判定。
