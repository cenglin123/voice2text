...
生成 `dist/voice2text-v0.1.4-windows-x64.zip` 和同名 `.sha256`。
构建会生成默认配置（校对关闭），不携带个人配置、GGUF 或开发环境。产物和模型不入 Git。

当前正式发行版为 `v0.1.4`，收录识别流就绪握手、准备期音频缓存、悬浮窗错误态自动复位、
系统 DPI 缩放、分发端诊断和连续讲话语义分段改进。发行 ZIP 为 471,874,103 字节，
SHA-256 为 `e113407a11a7e5fbb23863564b003468e1fb0a47c7de0e96a193cea34af604c7`。
本地构建与发布资产摘要一致；GitHub Release 标签指向提交 `cb51c61`。
