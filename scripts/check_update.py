...
def main() -> int:
    assert version_tuple("v0.1.10") > version_tuple("0.1.2")
    release = {
        "tag_name": "v0.1.3",
        "assets": [
            {"name": "voice2text-v0.1.3-windows-x64.zip",
             "browser_download_url": "https://github.com/cenglin123/voice2text/releases/download/v0.1.3/voice2text-v0.1.3-windows-x64.zip"},
            {"name": "voice2text-v0.1.3-windows-x64.zip.sha256",
             "browser_download_url": "https://github.com/cenglin123/voice2text/releases/download/v0.1.3/voice2text-v0.1.3-windows-x64.zip.sha256"},
        ],
    }
    version, zip_url, sha_url = select_assets(release)
    assert version == "0.1.3" and zip_url.endswith(".zip") and sha_url.endswith(".sha256")
    with tempfile.TemporaryDirectory(prefix="voice2text-update-check-") as tmp:
        root = Path(tmp)
        good = root / "good.zip"
        _archive(good, "0.1.3")
        validate_archive(good, "0.1.3")
        assert file_sha256(good) == hashlib.sha256(good.read_bytes()).hexdigest()
        bad = root / "bad.zip"
        _archive(bad, "0.1.3", malicious=True)
        try:
            validate_archive(bad, "0.1.3")
        except ValueError:
            pass
        else:
        ...
        prefix = "voice2text-v0.1.3-windows-x64/"
        ...
        result = subprocess.run(
            [str(powershell), "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script),
             ...
             "-Version", "0.1.3", "-NoRestart", "-KeepScript"],
